...
write2.py		- If directory doesn't exist, it is created
				- If it exists, opens existing directory & prints contents
			
write3.py		- If image directory doesn't exist, directory is created
...
write4.py		- If file doesn't exist, file is created
				- If exists, opens existing file reads & prints contents
				- Appends the names of each file in the folder of "file_path2" 

write5.py		- If file doesn't exist, file is created
				- If exists, opens existing file reads & prints contents
				- Appends the names of each file in the folder of "file_path2" 

write6.py		- Opens CSV file and copies dataset  
...
createFile.py	- Creates a file if it doesn't already exist and appends text to that file
getWiki.py		- Print the Wikipedia page text referred to in 'search'
key_press.py	- Example Code for keyboard input listener
open_gif.py		- Open a file (GIF) with it's default application

folder1.py		-> get list of folder names within a folder
				-> if existing folders:
...
usb0.py			- Checks for USB device, if found,
				- Moves the specified file (if exists) to the USB path
usb1.py			- Lists all attached USB modules by location and name 
usb2.py			- Doesn't seem to do anything, though no error?
